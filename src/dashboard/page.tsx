    ...
    const fetchTotalDistanceThisMonth = async () => {
      try {
        ...
        if (res.ok && data?.totalDistance !== undefined) {
          ...
          console.error("Failed to fetch total distance this month");
        }
      } catch (error) {
        ...
        >
          <Card className="relative overflow-hidden shadow-sm lg:py-1 lg:h-[150px]">
            ...
            <CardContent className="p-3 pt-0 md:p-4 md:pt-0">
              ...
              <p className="text-xs text-muted-foreground">
                {Object.entries(alertStats.typeCounts)
                  .map(
                    ([type, count]) => `${count} ${normalizeAlertType(type)}`
                  )
        ...
        >
          <Card className="relative overflow-hidden shadow-sm lg:py-1 lg:h-[150px]">
            ...
            <CardContent className="p-3 pt-0 md:p-4 md:pt-0">
              ...
              <p className="text-xs text-muted-foreground">
                {vehicleStats.idleVehicles} idle, {vehicleStats.movingVehicles}{" "}
                moving
              </p>
            </CardContent>
          </Card>
...
function AlertItem({
  ...
  return (
    <div className="flex items-start gap-3 pb-1">
      <div className={`rounded-full p-1.5 ${bg} flex-shrink-0`}>{icon}</div>
      <div className="space-y-0.5">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          {location ? ` ${location}` : "No location data available"}
        </p>
      </div>
    </div>
  );
...
function getIcon(type: string) {
  switch (type) {
    ...
    case "BATTERY_ALERT":
      return (
        <Battery className="h-3.5 w-3.5 text-amber-600 dark:text-amber-400" />
      );
    case "GEOFENCE_EXIT":
      return (
    ...
    default:
      return (
        <AlertTriangle className="h-3.5 w-3.5 text-gray-500 dark:text-gray-300" />
      );
  }
}
